...
for node in ocpi-{0..10}; do
	echo "$(ping -c 1 $node 2>/dev/null|grep ocp|grep PING|awk '{ print $2 }') openshift_node_labels=\"{'region': 'infra', 'zone': 'default', 'runtime': 'cri-o'}\" openshift_schedulable=True"
done|grep ocpi >>/etc/ansible/hosts
for node in ocpn-{0..30}; do
	echo "$(ping -c 1 $node 2>/dev/null|grep ocp|grep PING|awk '{ print $2 }') openshift_node_labels=\"{'region': 'nodes', 'zone': 'default', 'runtime': 'cri-o'}\" openshift_schedulable=True"
done|grep ocpn >>/etc/ansible/hosts
fi

echo "[glusterfs]" >>/etc/ansible/hosts
for node in ocpi-{0..10}; do
        echo $(ping -c 1 $node 2>/dev/null|grep ocp|grep PING|awk '{ print $2 }') glusterfs_ip=$(getent hosts $node|awk '{ print $1 }') glusterfs_devices=\'[ \"/dev/sde\", \"/dev/sdd\", \"/dev/sdf\" ]\'
done|grep ocpi >>/etc/ansible/hosts
